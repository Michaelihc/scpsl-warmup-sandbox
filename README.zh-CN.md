...
- 面向玩家的 `.help` 和 `.bots setcount <数量>` 命令带冷却限制。
- 服务器专属设置（Server Specific Settings）GUI 支持切换阵营、刷物品/弹药、传送辅助和修改机器人数量。
...
玩家可以打开 SCP:SL 的服务器专属设置（Server Specific Settings）使用 GUI。个人操作使用较短本地冷却；全局机器人设置使用服务器共享冷却。
