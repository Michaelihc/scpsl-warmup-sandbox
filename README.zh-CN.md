...
- 面向玩家的 `.help`、`.bots setcount <数量>` 和 `.panel` 命令带冷却限制。
- 玩家指令面板支持切换职业、刷物品/弹药、传送辅助和修改机器人数量。
...
- 这是人机练枪和热身服务器。
- 服务器会自动生成机器人。
- 所有玩家都有 `.panel` 玩家面板，可临时切角色、传送和发物品。
- 输入 `.help` 查看玩家命令。
- 服务器仍在公测开发中。
...
.bots setcount <数量>
.panel
.panel role <玩家ID|me> <职业>
.panel give <玩家ID|me> <物品>
.panel bring <玩家ID>
.panel goto <玩家ID>
.panel bots <数量>
```
...
`.panel` 会打开一个短时间指令面板提示。玩家可用它原地切换职业、刷常用物品/弹药、bring/goto 玩家，以及调整机器人数量。默认窗口为 20 秒，结束后进入全局冷却和个人冷却。如果修改面板窗口时长，个人冷却会按配置中的 20 秒基准等比例缩放。
